											...
											<table class="table table-striped" border='1'
												...
												<tr>
													...
													<td><select name="Runway" class="form-control">
															<option selected="selected" value="${runwayid}">${runwayname}</option>
															<c:forEach var="runway" items="${runway}">
																<c:forEach var="plan" items="${plan.runways}">
																	<c:if test="${runway.id == plan.id}">
												...
												<tr>
													...
													<td><select name="Stage" class="form-control">
															<option selected="selected" value="${stageid}">${stagename}</option>
															<c:forEach var="stage" items="${stage}">
																<c:forEach var="plan" items="${plan.stages}">
																	<c:if test="${stage.id == plan.id}">
												...
												<tr>
													<td>Checkpoint :</td>
													<td><textarea name="checkpoint" id="checkpoint"
															class="form-control">${checkpoint}</textarea>
													<script>
													$(function() {
													    $('#checkpoint').ckeditor({
													        toolbar: 'Full',
